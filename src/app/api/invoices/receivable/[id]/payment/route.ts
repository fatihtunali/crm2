  ...
  try {
    ...
    if (!invoiceId || isNaN(parseInt(invoiceId))) {
      return errorResponse(
        badRequestProblem('Invalid invoice ID', `/api/invoices/receivable/${invoiceId}/payment`)
      );
    }
    ...
    if (!payment_amount || isNaN(Number(payment_amount)) || Number(payment_amount) <= 0) {
      return errorResponse(
        badRequestProblem('Invalid payment amount', `/api/invoices/receivable/${invoiceId}/payment`)
      );
    }

    // Get current invoice
    const [invoice] = await query(
      'SELECT * FROM invoices_receivable WHERE id = ?',
      [invoiceId]
    ) as any[];

    if (!invoice) {
      return errorResponse(
        notFoundProblem(`Receivable invoice with ID ${invoiceId} not found`, `/api/invoices/receivable/${invoiceId}/payment`)
      );
    }
    ...
    if (newPaidAmount > totalAmount) {
      const response = errorResponse(
        conflictProblem(
          ...
          `/api/invoices/receivable/${invoiceId}/payment`
        )
      );
    ...
    return errorResponse(
      internalServerErrorProblem('Failed to record payment', `/api/invoices/receivable/${id}/payment`)
    );
  }
}
