...
export default function DirectClientsPage() {
  ...
  async function fetchClients() {
    try {
      // Filter for direct clients only
      const res = await fetch('/api/clients?client_type=direct_client&limit=1000', {
        headers: {
    ...
    <div className="p-8">
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Customers who booked directly with you</p>
        </div>
        <button
      ...
      <div className="mb-4 flex items-center justify-between">
        <p className="text-sm text-gray-600">
          Showing <span className="font-semibold">{filteredClients.length}</span> of <span className="font-semibold">{clients.length}</span> direct clients
        </p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-500">No direct clients found</div>
        ) : (
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="text-left p-4 font-semibold text-gray-700">Language</th>
            ...
            <tbody>
              {filteredClients.map((client) => (
                <tr key={client.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="p-4 text-sm text-gray-600 uppercase">{client.language_preference}</td>
                  <td className="p-4">
